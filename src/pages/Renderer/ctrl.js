yodasws.page('pageRenderer').setRoute({
	title: 'Renderer',
	template: 'pages/Renderer/Renderer.html',
	canonicalRoute: '/Renderer/',
	route: '/Renderer/?',
}).on('load', () => {
	let raceTrack;
	let environment;
	...
	// Load Race JSON
	document.querySelector('input[type="file"][name="race"]').addEventListener('change', (evt) => {
		...
		reader.onload = ((jsonFile) => (e) => {
			...
			// TODO: run buildReplay only after race and environment files have been uploaded
			if (environment instanceof Environment) {
				buildReplay(raceTrack, environment);
	...
	document.querySelector('input[type="file"][name="environment"]').addEventListener('change', (evt) => {
		const file = evt.target.files[0];
		if (file.type !== 'application/json') {
			evt.preventDefault();
			return;
		...
		reader.onload = ((jsonFile) => (e) => {
			...
			// TODO: run buildReplay only after race and environment files have been uploaded
			if (raceTrack instanceof RaceTrack) {
				buildReplay(raceTrack, environment);
...
// TODO: This is silly. Just use the sector numbers for the cameras to listen to instead of φ values, and dispatch events when cars enter new sectors. This would also allow for more precise camera switching and less overhead from constantly checking φ values.
const RaceEvents = (() => {
...
function buildReplay(raceTrack, {
	...
	// 1. event listeners on sector changes and other race events
	...
	const CameraPicker = (() => {
		...
		// TODO: Replace this with event listeners of race events
